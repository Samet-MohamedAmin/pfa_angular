...
export class TrainingDetailsComponent implements OnInit {
  ...
  getTrainingRequestState(){
    if(this.user) {
      if(this.training.attendees && this.training.attendees.includes(this.user._id)){
        this.trainingRequestState = 'ACCEPTED';
      }
      else {
        this.trainingService.getUserRequests(this.user._id).subscribe(
  ...
  registerRequest(){
    if(this.trainingRequestState === null){
      this.trainingService.requestRegistration(this.user.type,this.user._id, this.training._id).subscribe(
        obj => console.log(obj)
      );
